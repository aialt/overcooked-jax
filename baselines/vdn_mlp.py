...
def main(): 
    ...
    use_cnn = config.network_name == "cnn"
        ...
        def evaluate_on_environment(test_env, rng, train_state):
            ...
            def evaluation_step(step_state, unused):
                ...
                q_vals = jax.vmap(network.apply, in_axes=(None, 0))(
                    train_state.params,
                    batchify(last_obs, env.agents, flatten=not use_cnn),  
    ...
    if use_cnn:
        ...
        init_x = jnp.zeros((1, init_env.obs_size))
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                q_next_target = jax.vmap(network.apply, in_axes=(None, 0))(
                    train_state.target_network_params, batchify(minibatch.second.obs, env.agents, flatten=not use_cnn),
                ...
                def _loss_fn(params):
                    q_vals = jax.vmap(network.apply, in_axes=(None, 0))(
                        params, batchify(minibatch.first.obs, env.agents, flatten=not use_cnn)
                    )  # (num_agents, batch_size, ...)
